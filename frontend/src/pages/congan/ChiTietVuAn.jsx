...
export default function ChiTietVuAn() {
    ...
    if (!vuAn) return <div className="p-10 text-white">Không tìm thấy vụ án</div>;
    ...
    return (
        <div className="min-h-screen bg-[#0f1a26] p-8 text-white">
            ...
            <div className="mb-8 flex items-start justify-between">
                <div>
                    <h1 className="text-3xl font-bold uppercase text-[#ff5252]">{vuAn.tenvuan}</h1>
                    <p className="mt-1 text-gray-400">Mã vụ án: <span className="font-mono text-white">{vuAn.id_vuan}</span></p>
                </div>
            ...
            <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
                ...
                <div className="space-y-6 lg:col-span-2">
                    ...
                    <div className="rounded-xl border border-gray-700 bg-[#1b2838] p-6">
                        ...
                        <p className="whitespace-pre-line text-gray-300">{vuAn.mota}</p>
                        <div className="mt-6 grid grid-cols-2 gap-4 border-t border-gray-700 pt-6">
                            <div>
                                <p className="text-sm text-gray-500">Ngày khởi tố</p>
                                <p className="flex items-center gap-2 font-medium"><Calendar size={16} /> {new Date(vuAn.ngaytao).toLocaleDateString("vi-VN")}</p>
                            </div>
                            <div>
                                <p className="text-sm text-gray-500">Cán bộ thụ lý</p>
                                <p className="font-medium">{vuAn.nguoitao || "Admin"}</p>
                            </div>
                        </div>
                    </div>

                    <VictimList victims={vuAn.ds_nannhan} />
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-left text-sm text-gray-300">
                                    <thead className="bg-[#162436] text-xs uppercase text-gray-400">
                                        <tr>
                                            <th className="px-4 py-3">Mã NP</th>
                                    ...
                                    <tbody className="divide-y divide-gray-700">
                                        {vuAn.ds_nghipham.map((np) => (
                                            <tr key={np.id_nghipham} className="hover:bg-[#22344a]">
                                                <td className="px-4 py-3 font-mono text-[#4ECDC4]">{np.id_nghipham}</td>
                                                ...
                                                <td className="cursor-pointer px-4 py-3 text-blue-400 hover:underline" onClick={() => navigate(`/congan/nghipham/${np.id_nghipham}`)}>
                                                    Xem
                                                </td>
                                            </tr>
                                        ))}
                ...
                <div className="space-y-6">
                    <div className="rounded-xl border border-gray-700 bg-[#1b2838] p-6">
                        <h3 className="mb-3 text-lg font-bold text-gray-300">Nguồn gốc vụ án</h3>
                        {vuAn.dontogiac ? (
                            ...
                            <p className="text-sm italic text-gray-500">Vụ án được khởi tố trực tiếp.</p>
                        )}
                    </div>
                     ...
                     <div className="rounded-xl border border-gray-700 bg-[#1b2838] p-6">
                        <div className="mb-4 flex items-center justify-between">
                            <h2 className="flex items-center gap-2 text-xl font-bold text-[#f9ca24]">
                                <Paperclip size={20} /> Tài liệu/Chứng cứ
                            </h2>
                            <button onClick={() => setShowModal(true)} className="flex items-center gap-1 rounded-full border border-[#f9ca24]/50 bg-[#f9ca24]/20 px-3 py-1.5 text-sm font-medium text-[#f9ca24] transition hover:bg-[#f9ca24]/40">
                                <Plus size={16} /> Thêm
                            </button>
                        </div>
                        ...
                        {vuAn.ds_chungcu?.length > 0 ? (
                            <div className="space-y-3">
                                {vuAn.ds_chungcu.map((cc) => (
                                    <div key={cc.id_chungcu} className="group flex gap-3 rounded border border-gray-600 bg-[#0f1a26] p-2 transition hover:border-[#f9ca24]">
                                        <div className="relative flex h-16 w-16 items-center justify-center overflow-hidden rounded bg-gray-800 shrink-0">
                                            {["hình ảnh", "vật lý"].includes(cc.loaichungcu) || cc.duongdantaptin?.match(/\.(jpeg|jpg|png|gif)$/i) ? (
                                                <img src={getFileUrl(cc.duongdantaptin)} alt="evidence" className="h-full w-full object-cover" />
                                            ) : (
                                                <FileText size={24} className="text-gray-500" />
                                            )}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <p className="truncate text-sm font-bold capitalize text-white">{cc.loaichungcu}</p>
                                            <p className="line-clamp-1 text-xs text-gray-400">{cc.mota}</p>
                                            <a href={getFileUrl(cc.duongdantaptin)} target="_blank" rel="noreferrer" className="text-[10px] text-[#f9ca24] hover:underline mt-1 block">Xem chi tiết</a>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ) : (
                            <div className="text-center text-sm text-gray-500 py-4">Chưa có chứng cứ.</div>
                        )}
                    </div>
                </div>
            </div>
            
            <AddEvidence isOpen={showModal} onClose={() => setShowModal(false)} caseId={id} onSuccess={fetchCaseDetail} />
        </div>
    );
}